...
def fit_monthly(alldarks, orbit, verbose=False, debug_pixnr=None, short=False, give_errors=False, **kwargs):
    """
    ...
    for pixnr in range(n_pix):
        ...
        if np.sum(idx_fin) > min_degrees:
            ...
            if verbose:
                print(pixnr, dev_residual, "parameter errors:", fitobj.xerror, fitobj.message)

            # pass b : feedback stddev of residual as noise
            feedback = np.empty(n)
            feedback[:] = dev_residual
            fitobj = kmpfit.simplefit(scia_dark_fun2, p0, x, pix_readouts, err=feedback, ftol=1e-8, parinfo=parinfo)
            residual = scia_dark_fun2(fitobj.params, x) - pix_readouts
            dev_residual = np.std(residual)
            avg_residual = np.mean(residual)
...
if __name__ == "__main__":
    ...
    ret = fit_monthly(ad, orbit, verbose=True, debug_pixnr=621, short=False, give_errors=True)
