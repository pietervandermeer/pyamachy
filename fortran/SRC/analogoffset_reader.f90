MODULE analogoffset_reader
  ...
  ! read a single orbit of analog offset data (channel 8 only)
  ! orbit: absolute orbit [1..53000]
  ! file_name: valid file name of "interpolatedmonthlies" database
  SUBROUTINE read_analogoffset(orbit, file_name, data, data_errors, error) 
    INTEGER, INTENT(IN) :: orbit
    CHARACTER(LEN=*), INTENT(IN) :: file_name
    REAL(8), DIMENSION(:), INTENT(INOUT) :: data, data_errors
    ...
    CALL h5dread_f(dset_id, H5T_NATIVE_DOUBLE, data_errors, dim, error, file_space_id=filespace_id, mem_space_id=memspace_id)
    WRITE(*,*) "hyperslab read", dset_id, "error=", error
  ...
  END SUBROUTINE read_analogoffset
...
END MODULE analogoffset_reader
